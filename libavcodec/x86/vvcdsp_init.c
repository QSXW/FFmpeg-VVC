...
void ff_vvc_put_vvc_luma_hv_10_avx512icl(int16_t *dst, const uint8_t *_src, const ptrdiff_t _src_stride,
    const int height, const intptr_t mx, const intptr_t my, const int width,
    const int hf_idx, const int vf_idx);

void (*p_vvc_put_vvc_luma_hv_10_tt)(int16_t *dst, const uint8_t *_src, const ptrdiff_t _src_stride,
    const int height, const intptr_t mx, const intptr_t my, const int width,
    const int hf_idx, const int vf_idx);

static void vvc_put_vvc_luma_hv_10_wrap(int16_t *dst, const uint8_t *_src, const ptrdiff_t _src_stride,
    const int height, const intptr_t mx, const intptr_t my, const int width,
    const int hf_idx, const int vf_idx)
{
    ...
    {
        ff_vvc_put_vvc_luma_hv_10_avx512icl(dst, _src, _src_stride, height, mx, my, width, hf_idx, vf_idx);
    }
    else
    {
        p_vvc_put_vvc_luma_hv_10_tt(dst, _src, _src_stride, height, mx, my, width, hf_idx, vf_idx);
    }
}
...
void ff_vvc_dsp_init_x86(VVCDSPContext *const c, const int bit_depth)
{
    ...
    {
        p_vvc_put_vvc_luma_hv_10_tt = c->inter.put[LUMA][1][1];
        c->inter.put[LUMA][1][1] = vvc_put_vvc_luma_hv_10_wrap;
    }
}
